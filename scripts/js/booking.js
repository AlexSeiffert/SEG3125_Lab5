// Booking logic
import { loadBookings, saveBookings, KEY } from "./storage.js";
import {
  parseTimeToMinutes,
  minutesToTimeStr,
  isWithinHours,
...
function isPickupValidOrEmpty(earliestPickupMin) {
  ...
  return (
    pickupEl.checkValidity() &&
    isWithinHours(p, OPEN_MIN, CLOSE_MIN) &&
...
function createBookingOrError() {
  ...
  if (!isDateValid())
    return {
      error: "Please select a valid date (Mon–Sat only; no past dates).",
    };
  if (!isTimeValid()) {
    return {
      error: "Start time must be within 10:00–18:00 (15-minute options).",
    };
  }
  const startMin = parseTimeToMinutes(timeEl.value);
  const endMin = startMin + svc.durationMinutes;
  if (endMin > CLOSE_MIN) {
    return {
      error:
        "This service would end after 18:00. Please choose an earlier start time.",
    };
  }
  ...
  if (!isWithinHours(pickupMin, OPEN_MIN, CLOSE_MIN)) {
